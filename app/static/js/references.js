        ...
        switch(command) {
            ...
            case 'ok':
                if (typeof this.callback === 'function') {
                    ...
                    //alert(id+', item:'+reprObject(item)+', key:'+key+', value:'+value+', x:'+reprObject(x));
...
var $ConfigSelector = {
    ...
    set_blank: function(class_name, is_nodata, line) {
        var i = -1;
        var [selected, id] = makeTabLineAttrs(null, class_name, i);
    ...
    setContent: function(mode) {
        ...
        if (is_null(mode) || mode == 0) {
            ...
            if (this.backup.command == 'blank') {
                ...
                var [selected, id] = makeTabLineAttrs(data, class_name, i);
    ...
    confirmation: function(command) {
        var action = this.actions['default'];
        var x = command.split(DEFAULT_HTML_SPLITTER);
        var [service, command] = [x[0], x.length > 1 ? x[1] : command];
        var mode = this.get_mode();
        var id = this.rid || '';
